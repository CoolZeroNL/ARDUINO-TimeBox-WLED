...
void CallAPI(String Host, String Path, bool outputRGB, bool outputName) {
  WiFiClient client;
  ...
  Serial.print(Path + " [HTTP] begin...");
  if (http.begin(client, mainURL+Host+Path)) {  // HTTP
      ...
      Serial.printf("[HTTP] RESPONSE... code: %d \n", httpCode);
        ...
        if (outputName){
           if (httpCode == HTTP_CODE_OK || httpCode == HTTP_CODE_MOVED_PERMANENTLY) {
             ...
             doc.Parse( payload.c_str() );
              XMLElement* wled = doc.FirstChildElement( "vs" );
              const char* Name = wled->FirstChildElement( "ds" )->GetText();
              Serial.println(Name);
           }
        }
...
//https://github.com/mikerr/TTGO_ping/blob/main/ttgo_ping.ino
...
void scan() {
  ...
  Serial.println(ip_to_string(ip));
  Serial.println("Live IP addresses:");
  ...
  for (int i=1;i<255;i++) {
    ...
    if(Ping.ping(ip,1)) { 
        hosts[i] = 1; 
        Serial.println(ip_to_string(ip));
        CallAPI(ip_to_string(ip),"/win",false,true);
      } else { 
        hosts[i] = 0;
      }
   }
...
void setup() 
{
  ...
  Serial.println("Scanning for WLED instances:");
  scan();
  Serial.println("done...");
}
  ...
  if (!GreenSetAlready){
    if (LastTriggered != 0){
      if (diff_t > 3600){       // 60min
        // set color green
          CallAPI("192.168.178.129",ColorGreen,false,false);    // Set color to green   
  ...
  if ((millis() - lastDebounceTime) > debounceDelay) {
    ...
    if (switchStatus != buttonState) {
      ...
      if (buttonState == HIGH) {
        ...
        //CallAPI("/win",true,false);          // get & save current RGB value    
        
        LastTriggered = printLocalTime();              // save time..
        
        CallAPI("192.168.178.129",ColorPreset,false,false);    // Reset to Preset
